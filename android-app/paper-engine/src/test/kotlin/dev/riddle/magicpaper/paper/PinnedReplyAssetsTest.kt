...
class PinnedReplyAssetsTest {
    @Test fun `asset resolution is rooted in the paper engine module`() {
        val paperEngine = AndroidProjectTestPaths.paperEngine

        assertTrue(paperEngine.resolve("build.gradle.kts").isFile, paperEngine.path)
    }

    @Test fun `pinned reply assets exist with exact checksums and licenses`() {
        val paperEngine = AndroidProjectTestPaths.paperEngine
        val fonts = paperEngine.resolve("src/main/assets/fonts")
        val unicode = paperEngine.resolve("unicode-data/15.0.0")
        ...
        assertSha(paperEngine.resolve("src/test/resources/unicode/15.0.0/GraphemeBreakTest.txt"), "0d2080d0def294a4b7660801cc03ddfe5866ff300c789c2cc1b50fd7802b2d97")
    }
